...
<script setup lang="ts">
  ...
  const { data, columns, columnChecks, loading, pagination, handleSizeChange, handleCurrentChange, refreshData } = useTable({
    core: {
      apiFn: fetchTags,
      ...
      columnsFactory: () => [
        ...
        { prop: 'article_count', label: '使用次数', width: 120 },
        {
          prop: 'is_active',
          label: '状态',
          width: 100,
  ...
  const showDialog = (type: 'add' | 'edit', row?: any) => {
    ...
    if (type === 'edit' && row) {
      ...
      formData.value = { name: row.name, slug: row.slug }
    } else {
