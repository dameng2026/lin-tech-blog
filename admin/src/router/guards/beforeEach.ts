...
async function handleDynamicRoutes(
  ...
  try {
    console.log('[RouteGuard] Starting route initialization')
    // 1. 获取用户信息
    await fetchUserInfo()
    console.log('[RouteGuard] User info fetched')

    // 2. 获取菜单数据
    const menuList = await menuProcessor.getMenuList()
    console.log('[RouteGuard] Menu list fetched, count:', menuList.length)

    // 3. 验证菜单数据
    if (!menuProcessor.validateMenuList(menuList)) {
      throw new Error('获取菜单列表失败，请重新登录')
    }
    console.log('[RouteGuard] Menu list validated')

    // 4. 注册动态路由
    routeRegistry?.register(menuList)
    console.log('[RouteGuard] Routes registered')
    ...
    // 500 错误：通常表示服务器内部错误，可能是会话过期或服务器重启
    // 这种情况应该退出登录并跳转到登录页面，让用户重新登录
    if (isServerError(error)) {
      const userStore = useUserStore()
      console.warn('[RouteGuard] 服务器错误(500)，退出登录并跳转登录页')
      userStore.logOut()
      next({
        name: 'Login',
        query: { redirect: to.fullPath }
...
function isServerError(error: unknown): boolean {
  return isHttpError(error) && error.code === ApiStatus.internalServerError
}
