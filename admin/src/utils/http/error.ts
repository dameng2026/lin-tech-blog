...
export function handleError(error: AxiosError<ErrorResponse>): never {
  ...
  const errorMessage = responseData?.msg as string | undefined
  ...
  // 后端返回了具体错误消息时，优先展示它
  const hasBackendMessage = responseData && (responseData.msg || responseData.detail)
  const message = hasBackendMessage
    ? errorMessage
    : getErrorMessage(statusCode || ApiStatus.error)
